...
## Steady-Flow Adaptive Sampling (TF2 side study)

A separate TensorFlow 2 line of work on the **steady** mixed-form PINN (flow past
a cylinder), studying **residual-based adaptive collocation sampling** against a
fixed-collocation baseline. It is isolated from the PyTorch unsteady code under
[`src/steady/`](src/steady) (see [`src/steady/README.md`](src/steady/README.md)).

The adaptive sampler periodically resamples collocation points toward high-PDE-residual
regions. All three of its knobs are swept one-at-a-time off a baseline
(`nseed=5`, `acceptq=0.95`, `nexplore=1000`) in a single combined experiment:

| Parameter (env var) | Meaning | Swept values |
|---------------------|---------|--------------|
| `SWEEP_NSEED` | neighbors explored per accepted seed | 1, 2, **5**, 10 |
| `SWEEP_ACCEPTQ` | residual quantile above which points are accepted | 0.90, **0.95**, 0.97, 0.99 |
| `SWEEP_NEXPLORE` | global exploration points per resample | 500, **1000**, 2000, 5000, 10000 |

```bash
# Reference data: symlink the tracked Fluent steady solution into the CWD
ln -s results/reference/pinn_laminar_flow_original/fluent_reference FluentReferenceMu002

# Run the full sweep (all variants in parallel on one GPU), then plot
bash scripts/sweep_parallel.sh
python3 scripts/plot_sweep.py        # final loss + L2 error vs each parameter
python3 scripts/plot_loss_curve.py   # adaptive vs fixed collocation loss history
```

Lightweight evidence (per-variant CSVs, `summary.csv`, run logs) is tracked under
`results/experiments/steady_adaptive_sweep/`; heavy loss/weight pickles and the
generated figures stay untracked and are regenerated by the plot scripts above.
...
- `src/steady/`
  - TF2 steady-flow PINN study: `Steady_adpative.py` (adaptive collocation sampling),
    `Steady_ref.py` / `steady_org.py` (fixed-collocation baselines),
    `Steady_loss_anneal.py` (loss annealing), and `eval_l2.py` (relative L2 vs Fluent).
- `scripts/sweep_driver.sh`, `scripts/sweep_parallel.sh`, `scripts/sweep_run.sh`
  - Adaptive-sampling parameter-sweep drivers for `Steady_adpative.py`.
- `scripts/plot_sweep.py`, `scripts/plot_loss_curve.py`
  - Figure scripts for the adaptive-sampling sweep.
...
- `results/experiments/steady_adaptive_sweep/`
  - Adaptive-sampling sweep evidence: per-variant CSVs and `summary.csv` under
    `benchmarks/`, plus `adaptive`/`fixed` run logs under `logs/`.
