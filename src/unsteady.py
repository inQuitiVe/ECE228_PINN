...
def load_checkpoint(model, path, map_location):
    # weights_only=False: our checkpoints store history/optimizer state containing
    # numpy scalars; torch>=2.6 defaults weights_only=True and would reject them.
    # We trust our own checkpoints. (Kwarg accepted by torch>=1.13, so safe locally.)
