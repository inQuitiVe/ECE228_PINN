...
# Debug/runtime knobs. Set these before TensorFlow import so single-GPU
# machines such as RTX 3090 boxes expose logical GPU:0 cleanly.
os.environ.setdefault('CUDA_VISIBLE_DEVICES', '0')
PINN_TF_DEVICE = os.environ.get('PINN_TF_DEVICE', '/device:GPU:0')
...
class PINN_laminar_flow:
    ...
    def callback(self, loss, loss_f=None, loss_IC=None, loss_WALL=None, loss_INLET=None, loss_OUTLET=None):
        self.count = self.count+1
        if self.count == 1 or self.count % 10 == 0:
            if loss_f is None:
                print('L-BFGS %d | total=%.6e' % (self.count, loss))
            else:
                print('L-BFGS %d | total=%.6e f=%.6e ic=%.6e wall=%.6e inlet=%.6e outlet=%.6e'
                      % (self.count, loss, loss_f, loss_IC, loss_WALL, loss_INLET, loss_OUTLET))

    def log_losses(self, label, tf_dict):
        loss, loss_f, loss_IC, loss_WALL, loss_INLET, loss_OUTLET = self.sess.run(
            [self.loss, self.loss_f, self.loss_IC, self.loss_WALL, self.loss_INLET, self.loss_OUTLET],
            tf_dict)
        print('%s | total=%.6e f=%.6e ic=%.6e wall=%.6e inlet=%.6e outlet=%.6e'
              % (label, loss, loss_f, loss_IC, loss_WALL, loss_INLET, loss_OUTLET))
        return loss
    ...
    def train(self, iter, learning_rate):
        ...
        for it in range(iter):
            ...
            # Print
            if it % 100 == 0:
                self.log_losses('Adam %d' % it, tf_dict)
    ...
    def train_bfgs(self):
        ...
        self.log_losses('Before L-BFGS', tf_dict)
        self.optimizer.minimize(self.sess,
                                feed_dict=tf_dict,
                                fetches=[self.loss, self.loss_f, self.loss_IC,
                                         self.loss_WALL, self.loss_INLET, self.loss_OUTLET],
                                loss_callback=self.callback)
        self.log_losses('After L-BFGS', tf_dict)
    ...
    def getloss(self):  # To be updated
        ...
        print('Current loss | total=%.6e f=%.6e ic=%.6e wall=%.6e inlet=%.6e outlet=%.6e'
              % (loss, loss_f, loss_IC, loss_WALL, loss_INLET, loss_OUTLET))
...
if __name__ == "__main__":
    ...
    plt.savefig('./output/inlet_profile_debug.png', dpi=150)
    plt.close(fig)
    ...
    print('COLLOCATION_FRACTION:', COLLOCATION_FRACTION)
    print('LHS counts base/refine/lower/upper:', n_base, n_refine, n_wall, n_wall)
    XY_c = lb + (ub - lb) * lhs(3, n_base)
    XY_c_refine = [0.0, 0.0, 0.0] + [0.4, 0.4, tmax] * lhs(3, n_refine)
    XY_c_lw = [0.0, 0.0, 0.0] + [1.1, 0.02, tmax] * lhs(3, n_wall)
    XY_c_up = [0.0, 0.39, 0.0] + [1.1, 0.02, tmax] * lhs(3, n_wall)
    ...
    print('Final collocation shape:', XY_c.shape)
    ...
    print('Using TensorFlow device scope:', PINN_TF_DEVICE)
    with tf.device(PINN_TF_DEVICE):
        ...
        plt.savefig('./output/front_pressure_debug.png', dpi=150)
        plt.close()
